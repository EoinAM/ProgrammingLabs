	...
	, m_steering(0, 0)
{
	...
	m_seekCircle.setRadius(300);
	m_seekCircle.setOrigin(300, 300);
	m_seekCircle.setPosition(m_tankBase.getPosition());

}

////////////////////////////////////////////////////////////
void TankAi::update(Tank & playerTank, double dt, PickUp & t_health)
{
	...
	{
		sf::Vector2f vectorToHealth = seekHealth(t_health);
		m_steering += thor::unitVector(vectorToHealth);
		m_steering += collisionAvoidance();
		m_steering = MathUtility::truncate(m_steering, MAX_FORCE);
		m_velocity = MathUtility::truncate(m_velocity + m_steering, MAX_SPEED);
		break;
	...
	case AiBehaviour::STOP:
	{
		...
		//motion->m_speed = 0;
		if (!circleIntersect(playerTank.getBase()))
		{
			m_aiBehaviour = AiBehaviour::SEEK_PLAYER;
		}
		break;
	...
	rotateTurret(playerTank.getBase());
	...
	m_pool.update(dt, m_wallSprites, playerTank.getBase());
...
bool TankAi::circleIntersect(sf::Sprite t_playertank)
{
	float distanceBetween;
	distanceBetween = sqrt(((t_playertank.getPosition().x - m_seekCircle.getPosition().x) * (t_playertank.getPosition().x - m_seekCircle.getPosition().x) +
							(t_playertank.getPosition().y - m_seekCircle.getPosition().y) * (t_playertank.getPosition().y - m_seekCircle.getPosition().y)));

	if (distanceBetween <= m_seekCircle.getRadius())
	{
		return true;
	}
	else
...
sf::Vector2f TankAi::seekHealth(PickUp t_health)
{
	return t_health.getPosition() - m_tankBase.getPosition();
}

void TankAi::rotateTurret(sf::Sprite & t_player)
{
	float angle = std::atan2(t_player.getPosition().y - m_turret.getPosition().y, t_player.getPosition().x - m_turret.getPosition().x);
	angle /= Tank::DEG_TO_RAD;
...
void TankAi::requestFire()
{
	...
	if (m_shootTimer == s_TIME_BETWEEN_SHOTS)
	{
		...
		tipOfTurret.x += std::cos(Tank::DEG_TO_RAD * m_turret.getRotation()) *((m_turret.getLocalBounds().top + m_turret.getLocalBounds().height) * 1.7f);
		tipOfTurret.y += std::sin(Tank::DEG_TO_RAD * m_turret.getRotation()) * ((m_turret.getLocalBounds().top + m_turret.getLocalBounds().height) * 1.7f);
