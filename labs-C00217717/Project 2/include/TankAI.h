...
public:
	...
	void update(Tank & playerTank, double dt, PickUp & t_health);
	...
	bool collidesWithPlayer(Tank & t_player);

	bool m_fireRequested{ false };

	bool circleIntersect(sf::Sprite t_playertank);

	sf::Vector2f seekHealth(PickUp t_health);

	void rotateTurret(sf::Sprite & t_player);

	int m_shootTimer{ 1100 };
	static const int s_TIME_BETWEEN_SHOTS{ 100 };
...
private:
	...
	float m_turretRotation{50.0 };

	// Current velocity.
	sf::Vector2f m_velocity{ 10,10 };
