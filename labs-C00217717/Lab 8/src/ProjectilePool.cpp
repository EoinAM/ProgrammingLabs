...
Projectile const & ProjectilePool::getActive() const
{
	return m_projectiles.at(m_current);
}

void ProjectilePool::update(double dt, std::vector<sf::Sprite>& wallSprites, sf::Sprite & t_aitank)
{
	...
	{
		if (!m_projectiles.at(i).update(dt, wallSprites, t_aitank))
		{
		...
		if (m_projectiles[i].tankCollision)
		{
			AiTankCollision = true;
		}
	}
